...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    h1: ({ children }) => (
      <h1 className='mt-4 text-[4vw] font-extrabold text-center bg-linear-to-b from-primary/60 to-primary text-transparent bg-clip-text'>{children}</h1>
    ),
    h2: ({ children }) => {
    ...
    code: (props: any) => {
      ...
      return (
        <code className="mt-4 text-primary font-mono px-2 py-1 rounded text-sm">
          {children}
        </code>
      )
    },
